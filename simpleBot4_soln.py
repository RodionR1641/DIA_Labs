...
class Brain():
    ...
    def thinkAndAct(self, x, y, sl, sr, count):
        ...
        target = [targetGrid[0]*100+50,targetGrid[1]*100+50]
        print(target)
        ...
        if dr>dl:
            ...
            speedLeft = -2.0
            speedRight = 2.0
        if abs(dr-dl)<dl*0.2: #approximately the same
            speedLeft = 5.0
            speedRight = 5.0
...
def createObjects(canvas,follow):
    agents = []
    passiveObjects = []
...
def runOnce(follow):
    ...
    agents, passiveObjects, count = createObjects(canvas,follow)
